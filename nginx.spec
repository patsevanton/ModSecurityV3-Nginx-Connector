...
%define git_ModSecurity  https://github.com/SpiderLabs/ModSecurity
...
rm -rf modsecurity-3
git clone %{git_ModSecurity}.git modsecurity-3
tar zcvf ~/rpmbuild/SOURCES/%{name}-%{version}.tar.gz %{name}-%{version}
cd modsecurity-3
...
%{__install} -p -m 0644 %{SOURCE108}  %{buildroot}%{nginx_confdir}
...
/usr/local/modsecurity/include/modsecurity/actions/*.h
/usr/local/modsecurity/include/modsecurity/collection/*.h
/usr/local/modsecurity/include/modsecurity/*.h
/usr/local/modsecurity/lib/libmodsecurity.*
